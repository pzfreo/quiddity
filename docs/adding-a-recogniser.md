...
The declared-family migration removed four sites this list used to carry: the per-family block in
`tools/generate_capability_manifest.py`, now `ManifestEvidence` on the declaration; the entry in
`tools/recognition_snapshot.py`, now read from `__all__`; and the rosters in
`tests/test_inventory_agreement.py` and `tests/test_golden_data.py`, both now derived. (A third
roster, in `tests/test_recognition_result.py`, was derived at the same time and then returned as
a site for a different reason in #632, which is why the net is four.) If you last added a family
before 0.3.0 and expect to edit those, you no longer do.
...
Twelve paths is fewer than it sounds. Sizes below are from the gusset-rib change, `527446e`,
adjusted where the architecture has moved since.

**Two entries are most of it, and both are review gates:**

- **`_effective_surfaces.py` (~20 lines).** Nominally conditional, usually needed: **24 of the 29
  family modules have an entry**, including gussets. Only `bosses`, `oriented_slots`, `passages`,
  `section_recesses` and `slots` have none. It is not restricted to families reading raw
  topology — a module using the run-owned surface query still records its disposition. Each entry
  says *why* its reads are what they are, which is why it cannot be derived. Budget for it.
- **`test_architecture.py` (~16 lines).** Chiefly the seam entry: what your module may import.
  Deriving that from your actual imports would approve whatever you wrote and check nothing.

**Two are mechanical restatements of your declaration:** `result.py` (~10 lines — the field, its
record-type import, and a `#:` doc comment) and `__init__.py` (~12 lines — the record types as
well as the recogniser).

**Two are a handful of lines each.** `_registry.py` is 2–3: the module in the import block, the
`DEFINITION` line in execution order, and a `PATTERNS` line if you have a pattern family. It was
43 before the migration moved the declaration into your module. `tests/test_recognition_result.py`
is about 7: a module import, the stub registering what your family is handed, and its `expected`
entry.

**Two are genuinely one line:** the `FamilyId` member and the census binding. Both look like
duplication and are not. `tests/test_registry.py` already cross-checks the census key against
your declaration, so the only thing the binding carries that the declaration does not is its
*position* — and `feature_census()` returns a `dict` whose key order is observable. The same is
true of the registry's order line: only four of the 33 families declare a dependency, so the rest
of the execution order is a choice rather than a derivation.

**The rest is prose and pins:** the `capabilities.md` rows (7 lines), and the three pinned test
rosters — `test_registry.py` 5, `test_golden_fixtures.py` 2, `test_mfcadpp_corpus.py` 1 — of
which the golden pin fails on a new family deliberately, so that someone looks at what it found
on the reference parts.

That is 85 lines across the twelve paths:

| | | | |
| --- | ---: | --- | ---: |
| `_effective_surfaces.py` | 20 | `census.py` | 1 |
| `test_architecture.py` | 16 | `_candidates.py` | 1 |
| `__init__.py` | 12 | `test_mfcadpp_corpus.py` | 1 |
| `result.py` | 10 | `test_golden_fixtures.py` | 2 |
| `test_recognition_result.py` | 7 | `test_registry.py` | 5 |
| `docs/capabilities.md` | 7 | `_registry.py` | 3 |

Against 637 for the gusset family's own module and tests, and a further 43 for its two golden
fixture parts, which are build123d modelling rather than registration. The `expected.json`
snapshots and `capabilities.json` are regenerated by tooling and are not hand edits, despite
dominating the diff.
