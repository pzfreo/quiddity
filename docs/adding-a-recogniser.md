...
| 2 | `src/quiddity/_registry.py` | one line placing the module's `DEFINITION` (and `PATTERNS`) in execution order; the declaration itself lives in the family module, see `gussets.py` | the registry validator |
