...
| `recognise_holes` | Coaxial internal full-cylinder stacks with through/flat/drill-point/unknown bottoms and injected countersink composition. Separated same-diameter stacks recombine only across a shared original internal cylindrical segment meeting both facing ends, or a proved shoulder transition; exterior gaps between lugs retain separate occurrences even in one solid. Existing contiguous-range tolerances remain unchanged. | Slot end caps, partial cylinders, unsupported interruptions without a common cylindrical source or shoulder transition, far-side counterbores, and automatic countersink rediscovery when none is injected. | Hole/counterbore/cross-bore goldens; separated-lug multiplicity, exact evidence, scale/placement/STEP and native/recovered split-cylinder regressions (#593). |
