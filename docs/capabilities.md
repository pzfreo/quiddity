...
| `recognise_sheet_metal_bodies` | Developable planar and cylindrical two-sided sheets with constant thickness. Reports paired skins, thickness-proved cut faces, planar flange regions, cylindrical bends and neutral-axis allowances for a caller-specified k-factor. The rooted bend graph must be a tree. The flat pattern contains triangulated source flange faces and developed bend strips in one 2D frame, with significant overlaps refused. Local partially paired curved forms are separate child records. | Native planes and cylinders only. Unexplained blind pockets are refused. The flat layout uses 0.1 model-unit tessellation and a thickness-scaled overlap tolerance; formed-feature children are not flattened. Material k-factor is supplied, not inferred. | Generated 4 mm bracket, blind-pocket refusal, aggregate and document checks; committed TTT hanger STEP with a checked main blank and two formed tab children. |
