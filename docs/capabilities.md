...
| `recognise_thin_wall_bodies` | Valid solids with a dominant constant thickness proven by reciprocal face-normal rays through material. Reports original face pairs, unpaired faces, paired area, and rim regions. A separate `ShellHistoryHint` estimates inward direction, outer and inner faces, opening rims, and pre-shell collars or post-shell cuts where the topology supports those interpretations. In aggregate recognition the wall supersedes plates, bosses, and risers wholly contained in its paired skins. | The history hint is explicitly heuristic: the same finished solid can arise from different shell directions and operation orders. Unoriented or unclassified faces stay out of the hint. Small or incomplete paired regions and bodies with no dominant thin wall are refused. | Synthetic box and pipe controls, imported cgb241/cgb207 STEP regressions, rotation and compound controls, reconciliation, aggregate face evidence and document projection tests. |
