...
| `recognise_sheet_metal_bodies` | Developable planar and cylindrical two-sided sheets with constant thickness. Reports propagated skins, cut-edge faces, planar flange regions, cylindrical bend pairs, inner radii, bend angles, axes and neutral-radius allowances for a caller-specified k-factor. A rooted bend graph references the source faces so a consumer can unfold them. | Only native planar and cylindrical surfaces with enough paired-skin and cut-edge evidence are accepted. The flat-pattern plan does not claim an overlap-free manufacturing blank. Extra bend connections are reported as cycle constraints; material k-factor is supplied, not inferred. | Generated 4 mm formed bracket with a 90 degree bend, plain-solid refusal, aggregate and document projection; local TTT hanger STEP validation when available. |
