...
| `recognise_circular_face_patterns` | Three to sixteen connected source-face groups on one valid solid whose sampled surfaces repeat about one proved native or principal axis. The record gives the axis, count, pitch, seed index and sampled fit error; evidence gives each exact face group, including split B-spline blades. | Partial cycles, disconnected groups, unequal per-instance surface-type areas, fits outside the bounded mesh tolerance, invalid solids and models above the explicit face or mesh budget. It does not infer a feature name or construction history. | Seven swept-fin synthetic positive and missing-fin negative; generated cgb203 impeller STEP regression, face-group/document attribution and runtime bound. |
...
| `recognise_gusset_ribs` | Material-filled right-triangular ribs joining two perpendicular principal planes, including cylindrical fillets along either or both slanted side edges. Each record gives thickness bounds, both support planes, signed virtual sharp-corner leg lengths and unambiguous body correlation in the supplied frame. | Fillets that remove either complete support leg, split or curved end caps, oblique support planes, and unproved or cross-solid material. | Gusset-rib golden, single-rib, rounded and void controls, STEP and framed recognition tests. |
| `recognise_gusset_rib_patterns` | Derived mirror pairs of two congruent ribs centred on their body's thickness-axis envelope, and constant-pitch arrays of at least three congruent ribs on one unambiguous body. Arrays take precedence over pair relations for their members. | Uneven spacing, off-centre two-rib placement, overlapping ribs, ambiguous body identity, and patterns spanning separate solids. | Symmetric pair, repeated row, off-centre issue fixture, uneven row and compound controls. |
...
| `recognise_hole_patterns` | Same-spec hole bolt circles and complete rectangular grids on one opening plane, plus constant-pitch linear arrays in world 3-D; greedy largest-first ownership. | Pairs, incomplete lattices as grids, uneven circles/rows, mixed specs, and a hole belonging to multiple returned patterns. | Bolt-circle/grid golden, separate-plane and 3-D-row regressions, pattern regressions, and scaling sentinel. |
| `recognise_holes` | Coaxial internal full-cylinder stacks with through/flat/drill-point/unknown bottoms and injected countersink composition. Nonparallel crossing bores remain independent stacks. Separated same-diameter stacks recombine only across a shared original internal cylindrical segment adjoining both lands and containing the bore-axis gap, or a proved shoulder transition; exterior gaps between lugs retain separate occurrences even in one solid. Existing contiguous-range tolerances remain unchanged. | Slot end caps, partial cylinders, unsupported interruptions without a common cylindrical source or shoulder transition, far-side counterbores, and automatic countersink rediscovery when none is injected. | Hole/counterbore/cross-bore goldens; separated-lug multiplicity, exact evidence, scale/placement/STEP oblique/offset crossings, lateral-contact and rounding-boundary controls, and native/recovered split-cylinder regressions (#593/#597). |
| `recognise_section_recesses` | Unified accepted pockets, passages, edge-open recesses, blind slots and channels with free 3-D frame, physical line/arc profile, explicit end conditions and result-local face/body evidence. Includes independently proved corner and partial-support channel profiles, plus native circular-seat channels up to 180° with exact cylindrical support and three proved openings. | Unproved extent summaries (explicit refusal in the document), tapered or stepped cavities, islands, ambiguous ownership, and unsupported interruptions. Circular-seat discovery excludes spline trims, undercuts beyond 180° and nonplanar axial ends. The schema does not make every detector free-axis. | Authored reconstruction, signed-axis, STEP, support/obstruction rejection, pattern and ownership tests; migration checks on authored and vendored MFCAD++ geometry. |
...
| `recognise_polygonal_bosses` | Attached regular square or hexagonal principal-axis bosses with four or six outward side faces, one A/F value, and two unambiguous terminal boundaries whose normals agree on the signed attachment direction. `axis` is X, Y or Z; `base`/`top` remain ascending coordinates even when attachment runs in the negative direction. Complete native constant-radius convex cylindrical corner-blend cycles may bridge the retained planar side ring when issuer-owned provenance is unambiguous. | Non-principal axes in the supplied recognition frame, other side counts, whole-stock prisms, inward recesses, incomplete or competing blend cycles, automatic collapse, and cross-solid assemblies. | Signed X/Y/Z, in-plane/arbitrary-rigid-motion, exact face-anchor, compound, STEP, square/hex blend cycles, aggregate and capability-negative tests. |
...
| `recognise_slot_patterns` | Constant-pitch linear and complete rectangular arrays of identical through `Slot` records on the same through plane, including identical proved end/corner radii. | Bolt circles, pairs, mixed sizes/planes/radius semantics, and incomplete grids. | Straight/obround-slot golden and pattern-negative tests. |
| `recognise_slots` | Principal-axis enclosed through-slots proved by opposed walls or qualifying obround end caps, independently per solid. A planar pair must have agreeing AAG arcs into shared boundary neighbours, or belong to one smooth-connected boundary component when STEP has fragmented that boundary (the gAAG-equivalent query); after graph-proved curved end interruptions are trimmed, its unrounded rectangular prism must be materially empty. Two proved semicircular caps publish `end_radius`; four equal proved quarter-cylinder corners publish `corner_radius` and the overall footprint span. `None` means unproved, not square. A connected curved region smoothly closing either selected depth end refuses the alternate deep-pocket interpretation. | Floored pockets and their alternate orthogonal projections, open-ended channels, internally oblique or merely narrow envelope sections, internal islands/bridges that the simple record cannot express, partial or mixed-radius corner sets, cross-solid composites, and opposed pairs assembled from different sides of a polygonal void. Aggregate reconciliation gives complete pocket and non-rectangular passage rings precedence over paired-wall fragments. | Signed X/Y/Z and framed covariance, straight/obround-slot golden, native/STEP rounded-corner proof, scale/partial/mixed-radius controls, split-smooth-closure, AAG-coherence mutation, H/U/thin-rib/scale adversaries, frozen MFCAD++ holdout, NIST corrections, recess-reconciliation regressions, and [paired MFCAD++-500 validation](benchmarks/e5-slot-depth-closure-validation.md). |
...
| `recognise_thin_wall_bodies` | Valid solids with one or more locally constant thicknesses proven by reciprocal face-normal rays through material. Each original face pair reports its measured thickness, including narrow cylindrical, toroidal and B-spline joint blends; unpaired faces each receive a cut-edge, joint-blend or residual non-wall class. A separate `ShellHistoryHint` estimates inward direction, outer and inner faces, opening rims, and pre-shell collars or post-shell cuts where the topology supports those interpretations. In aggregate recognition the wall supersedes plates, bosses, and risers wholly contained in its paired skins. | The history hint and unpaired-face classes are heuristic: the same finished solid can arise from different shell directions and operation orders, and a residual class does not prove a boss/rib feature type. The two main connected skins must carry a majority of paired area, and pairs must cover at least 85% of body face area; [cgb249 currently reaches only 33%](benchmarks/thin-wall-multiple-thickness-cgb249.md) and is refused. | Synthetic one- and two-thickness shells, rounded-plate refusal, box and pipe controls, imported cgb241/cgb207 STEP regressions, joint-round pairs, rotation and compound controls, reconciliation, aggregate face evidence and document projection tests. |
| `recognise_freeform_surfaces` | Native B-spline faces on a supplied part, whether another recogniser claimed them or not. Publishes untrimmed support parameters, direct same-support or G1 joins, connected continuity groups, and measured offset links inherited from proved wall pairs. | Native B-splines only. Degree-one sweeps can prove ruled or constant-vector extrusion construction; other B-splines do not establish unique loft history; offset links are local reciprocal-ray measurements, not global exact-offset proofs. Other native freeform classes remain outside this bounded family. | [cgb241](benchmarks/freeform-cgb241.md): ten source faces, four large outer support reconstructions, paired inner skins and continuity checks; independent unclaimed B-spline face. |
| `recognise_sheet_metal_bodies` | Predominantly developable two-sided sheets with measured thickness, planar flange regions, cylindrical bend groups, neutral-axis allowances and source-linked rounded or chamfered cut contours. Proved tabs are included in the flange/bend graph. A flat traversal is retained with explicit overlap witnesses when it cannot be a valid single blank. | Unexplained blind pockets remain refused. `flat_pattern` is absent when the full bend graph cannot be unfolded, with `flat_pattern_status` stating the limit; a plan with `valid_blank=False` is not a manufacturable blank. The 0.1 model-unit tessellation and k-factor are explicit. | Generated 4 mm bracket, blind-pocket refusal and aggregate/document checks; [cgb245 and TTT hanger](benchmarks/sheet-metal-cgb245-and-hanger.md) retain edge and tab geometry with bounded flat-plan status. |
...
| `CircularFacePattern` | One geometry-only rotational repetition of connected source-face groups with an axis, count, pitch, seed index and sampled fit error; exact source-face instance membership is published by the evidence view. |
...
| `GussetRibMirrorPair` | Two congruent ribs reflected across a body-centred principal plane. |
| `GussetRibArray` | Three or more congruent ribs at constant pitch along their common thickness axis. |
...
| `ThinWallBody` | One body-level wall reading with dominant thickness for compatibility, local original-face pairs carrying their own measured thickness, classified unpaired faces, cut-rim regions, paired-area coverage, and a nested heuristic history hint. |
| `FreeformSurface` | One source B-spline face, including untrimmed native support control data, its original face index, direct smooth or same-support joins, and a continuity group. A two-section degree-one sweep proves a ruled surface; equal translated sections prove a linear extrusion and its vector. Paired skin offsets use reciprocal material-ray evidence and are local measurements, not a global exact-offset theorem. Generic B-splines carry no inferred loft history. |
| `BSplineSurfaceSupport` | Native U/V degrees, complete control poles and weights, knots and multiplicities, and periodic flags, sufficient to reconstruct the untrimmed B-spline support. Trimming wires remain on the original source face. |
| `SurfaceContinuityLink` | One shared-edge relation to another freeform face: `same_support` or geometrically proved `G1`. No `G2` claim is made without separate proof. |
...
| `PolygonalBoss` | One attached regular square or hexagonal principal-axis boss, including all-corner rounded variants; `axis` is `"x"`, `"y"` or `"z"`, `side_count` is 4 or 6, and `base`/`top` are ascending coordinates along that axis. |
...
| `SectionPassage` | Root-exported nested source geometry for `OrientedSlot`, schema version 2 under `oriented-slots`; no standalone recogniser, aggregate field or census family is restored. |
| `PassageEnds` | Root-exported nested end topology of `SectionPassage`, schema version 2 under `oriented-slots`; retains explicit open-end flags and termination gradients. |
...
| `Slot` | One enclosed through-slot; no floor and no open longitudinal end. Schema 2 appends optional millimetre `end_radius` (two proved semicircular ends) and `corner_radius` (four proved uniform corners); `None` is unknown/unproved. Legacy `Pocket` carries the same compatible fields outside the current manifest. |
...
the pocket's physical depth. Untreated straight passages also require observed stock planes
at both ends; a thin empty probe at a blend tangent is insufficient (#556).
