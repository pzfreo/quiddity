...
def _run_case(part: Any, enabled: bool) -> tuple[Any, float]:
    ...
    original = family._discover_rectangular_blind_slots
    if not enabled:
        family._discover_rectangular_blind_slots = lambda *_args, **_kwargs: []
    try:
        started = time.perf_counter()
        product = _take_inventory(part)
        return product, time.perf_counter() - started
    finally:
        family._discover_rectangular_blind_slots = original
