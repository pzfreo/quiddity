    ...
    completed = subprocess.run(
        [
            ...
            (
                ...
                "rejected_view = v.build_recognition_evidence("
                "Box(60, 40, 20) - Box(30, 10, 30)); "
                "assert len(rejected_view.rejected_candidates) == 1; "
                "candidate = rejected_view.rejected_candidates[0]; "
                "assert isinstance(candidate, v.CandidateRef); "
                "assert rejected_view.candidate_family(candidate) == 'passages'; "
                "assert rejected_view.candidate_outcome(candidate) is "
                "r.RecognitionOutcome.REJECTED; "
                "assert rejected_view.candidate_reason(candidate) is "
                "r.ReconciliationReason.PASSAGE_SUPERSEDED_BY_SLOT; "
                "assert rejected_view.candidate_defining_faces(candidate); "
                "assert len(rejected_view.related_candidates(candidate)) == 1; "
