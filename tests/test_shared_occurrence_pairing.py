...
over every adjacent pair of every vendored corpus part. The second is that the scan is really
gone, which the operation-count sentinel at the bottom pins.
"""
...
import pytest
from build123d import Box, Cylinder, Sphere, Torus
...
def _scanned_pairing(graph: FaceGraph, a: FaceNode, b: FaceNode) -> list[tuple]:
...
def _assert_pairs_identically(graph: FaceGraph, a: FaceNode, b: FaceNode) -> int:
...
def test_every_adjacent_pair_of_every_corpus_part_pairs_identically() -> None:
    ...
    All 87 parts, every adjacent pair of every graph: 12,286 pairs and 12,826 issued
    occurrences, in about four seconds -- so there is no reason to sample.
    ...
    for path in CORPUS_PARTS:
        ...
        for node in graph.nodes:
            for neighbour in graph.neighbours(node):
                ...
                seen_occurrences += _assert_pairs_identically(graph, node, neighbour)
                seen_pairs += 1
    assert seen_pairs == 12286, seen_pairs
    assert seen_occurrences == 12826, seen_occurrences


@pytest.mark.parametrize("build", [lambda: Box(10, 10, 10), lambda: Cylinder(5, 10)])
def test_built_geometry_pairs_identically_in_both_directions(build) -> None:
    ...
    for node in fresh.nodes:
        for neighbour in fresh.neighbours(node):
            seen += _assert_pairs_identically(fresh, node, neighbour)
    assert seen  # the fixture would be vacuous with nothing shared


@pytest.mark.parametrize(
    ("build", "expected_seams"),
    [(lambda: Cylinder(5, 10), 1), (lambda: Sphere(5), 1), (lambda: Torus(10, 3), 2)],
)
def test_a_seam_edge_groups_with_its_own_reversed_reading(build, expected_seams) -> None:
...
def test_grouping_agrees_with_the_scan_on_every_face_of_a_real_part(sentinel_graph) -> None:
    ...
    assert len(sentinel_graph.nodes) == 664
    assert widest == 120  # the face whose L² scan the analysis named
