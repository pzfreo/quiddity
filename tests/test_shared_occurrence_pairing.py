...
def test_every_adjacent_pair_of_every_corpus_part_pairs_identically() -> None:
    ...
    All 91 parts, every adjacent pair of every graph: 14,031 pairs and 14,669 issued
    ...
    for path in CORPUS_PARTS:
        ...
        for node in graph.nodes:
            for neighbour in graph.neighbours(node):
                ...
                seen_pairs += 1
    assert (seen_pairs, seen_occurrences) == (14031, 14669), (
        f"the sweep visited {seen_pairs} adjacent pairs and paired {seen_occurrences} "
        "occurrences; 14031 and 14669 are what the vendored corpus holds. A drop means the "
